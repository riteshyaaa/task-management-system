...
export const TeamPage: React.FC = () => {
  ...
  const handleAddMember = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!currentTeam || !userEmail.trim()) return;
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-white flex items-center gap-2">
            Team Workspace & Access Control <Users className="w-5 h-5 text-indigo-400" />
          </h1>
          <p className="text-xs text-slate-400">
            Manage multi-tenant workspace members, granular RBAC assignments, and team settings
          </p>
        </div>
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-2xl p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-indigo-600/20 border border-indigo-500/40 flex items-center justify-center text-indigo-400 font-bold text-lg">
              {(currentTeam.name || 'T').charAt(0).toUpperCase()}
            </div>
            <div>
              ...
              <p className="text-xs text-slate-400 mt-0.5">
                {currentTeam.description || 'Enterprise workspace tenancy'}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 text-xs">
            ...
            <div className="text-center px-4 py-2 rounded-xl bg-slate-950 border border-slate-800">
              ...
              <span className="text-base font-bold text-slate-300">{formatDate(currentTeam.createdAt)}</span>
            </div>
          </div>
        </div>
      )}
